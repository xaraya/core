...
function replace( $matches )
{
    ...
    foreach ($list1 as $key => $value) {
        ...
        $text .= " NULL, XARVAR_DONT_SET)) {return;}";
    }
    return $text;
    ...
    while ($entry = readdir($dh)) {
        if (is_dir("$path/$entry")) {
            if (($entry != '.') &&
                ...
                //Dont follow symlinks to the modules repository
                if (is_link("$path/$entry")) continue;
            ...
            if (substr($entry, strlen($entry)-4) == '.php') {
                ...
                $new_file_contents = preg_replace_callback( "|(\\$[a-zA-Z0-9_]+)\s*=\s*xarVarCleanFromInput\s*\(([^)]*)\)\s*;|mU", 'replace', $file_contents );
