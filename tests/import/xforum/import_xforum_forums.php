 ...
 */
 //
   ...
   // Get datbase setup
    ...
    $query = 'SELECT fid, name, lastpost,description, displayorder, posts, threads, fup
    ...
    $catinfo = array();
    while (!$result->EOF) {
        list($id, $name, $lastpost, $descr, $order, $fposts, $fthreads, $fup) = $result->fields;
        if (!isset($catids[$fup])) {
       ...
       //Let's try and get the last poster and last post date
       $lastposted =array();
       $lastposted = explode('|',$lastpost);
       $lastposttime =(isset($lastposted[0]) && !empty($lastposted[0])) ? date('Y-m-d G:i:s',$lastposted[0]):date('Y-m-d G:i:s');
       ...
       // heh - just modifed the function so we can pass in values
          $forumid[$id]=xarModAPIFunc('xarbb',
                               ...
                               array('fname'    => $name,
                                     ...
                                     'fposts'   => $fposts,
                                     'fposter'  => $lastposter,
                                     'fpostid'  => $lastposttime)); //Set all forum creation to Admin
