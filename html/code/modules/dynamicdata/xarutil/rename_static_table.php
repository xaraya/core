    ...
    function dynamicdata_util_rename_static_table()
    {
        ...
        if ($data['confirm']) {
            if (empty($data['newtable'])) 
                xarController::$response->redirect(xarModURL('dynamicdata','util','view_static',array('table' => $data['table'])));
            $query = 'RENAME TABLE ' . $data['table'] . ' TO ' . $data['newtable'];
            $dbconn = xarDB::getConn();
            $dbconn->Execute($query);

            // Jump to the next page
            xarController::$response->redirect(xarModURL('dynamicdata','util','view_static',array('table' => $data['newtable'])));
            return true;
        }
        return $data;
    }
