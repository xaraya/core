...
class SubItemsProperty extends DataProperty
{
    ...
    public $prefixarray       = [];
    public $itemsdata         = [];                   // holds the subitem objects
    public $toupdate          = [];                   // holds the ids of items to update
    ...
    */
    ...
    public function checkInput($name = '', $value = null)
    {
        ...
        $this->objectref->getItem(['itemid' => $this->objectref->itemid]);
        ...
        foreach ($itemids as $prefix) {
            ...
            $itemsdata[$newprefix][$prefix] = $data['object']->getFieldValues([], 1);
        }
    ...
    public function showInput(array $data = [])
    {
        ...
        if (!empty($data['object'])) {
            $this->initialization_refobject = $data['object'];
        }
        if (empty($data['addremove'])) {
            $data['addremove'] = $this->initialization_addremove;
        }
        if (empty($data['minimumitems'])) {
            $data['minimumitems'] = $this->initialization_minimumitems;
        }
        ...
        foreach ($data['object']->getProperties() as $name  => $property) {
            if (!isset($this->defaultvalues[$name])) {
                $data['defaultfieldvalues'][$name] = $property->defaultvalue;
            } else {
                $data['defaultfieldvalues'][$name] = $this->defaultvalues[$name];
            }
        }
        ...
        if (empty($data['items'])) {
            ...
            if ($data['minimumitems'] > count($data['items'])) {
                ...
                for ($i = $start;$i < $limit;$i++) {
                    $data['items'][$i + 1] = $data['defaultfieldvalues'];
                }
            }
            /*
        ...
        if (isset($data['layout'])) {
            $data['object']->layout = $data['layout'];
        }
        if (isset($data['tplmodule'])) {
            $data['object']->tplmodule = $data['tplmodule'];
        }
