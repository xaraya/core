...
function dynamicdata_user_filtertag(array $args = [])
{
    ...
    if ($filter_submitted) {
        if (!xarVar::fetch('objectname', 'str', $objectname, '', xarVar::NOT_REQUIRED)) {
            return;
        }
        if (!xarVar::fetch('filtername', 'str', $filtername, '', xarVar::NOT_REQUIRED)) {
            return;
        }
        if (!xarVar::fetch('return_url', 'str', $return_url, '', xarVar::NOT_REQUIRED)) {
            return;
        }
        if (!xarVar::fetch('name', 'array', $names, [], xarVar::NOT_REQUIRED)) {
            return;
        }
        if (!xarVar::fetch('source', 'array', $source, [], xarVar::NOT_REQUIRED)) {
            return;
        }
        if (!xarVar::fetch('op', 'array', $op, [], xarVar::NOT_REQUIRED)) {
            return;
        }
