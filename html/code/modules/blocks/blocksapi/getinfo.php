...
class GetinfoMethod extends MethodClass
{
    ...
    public function __invoke(array $args = [])
    {
        ...
        if (empty($args['instance']) && empty($args['type'])) {
            ...
            $vars = ['type or instance','blocks', 'blocksapi', 'getinfo'];
        ...
        if (isset($args['type_state'])) {
            if (!empty($args['type_state']) && is_string($args['type_state'])) {
                $args['type_state'] = [$args['type_state']];
            }
        ...
        if (!empty($invalid)) {
            ...
            $vars = [join(', ', $invalid), 'blocks', 'blocksapi', 'getinfo'];
        ...
        if (!empty($args['template'])) {
            if (strpos($args['template'], ';') !== false) {
                [$box_template, $block_template] = explode(';', $args['template']);
            } else {
        ...
        foreach ($content as $k => $v) {
            if (in_array($k, $to_skip) || !isset($args[$k])) {
                continue;
            }
            $datatype = gettype($v);
            switch ($datatype) {
