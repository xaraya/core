...
/**
 * blocks instancesapi getitems function
...
class GetitemsMethod extends MethodClass
{
     ...
     * @return array|void Array of items fetched.
     * @throws \BadParameterException
     * @see InstancesApi::getitems()
     */
    public function __invoke(array $args = [])
    {
        ...
        if (isset($module)) {
            if (empty($module)) {
                ...
                || (!$this->mod()->isAvailable($module)
                    && (!$this->mem()->has('Blocks.event', 'modremove') || $this->mem()->get('Blocks.event', 'modremove') != $module))
            ) {
                $invalid[] = 'module';
            } else {
        ...
        if (!empty($invalid)) {
            ...
            $vars = [join(', ', $invalid), 'blocks', 'instancesapi', 'getitems'];
