...
class CategoryWorker extends xarObject
{
    ...
    {
        if (empty($id)) {
            throw new Exception(xarML('No id passed to id2name'));
        }
    ...
    {
        if (empty($name)) {
            throw new Exception(xarML('No name passed to name2id'));
        }
        ...
        $result = $q->output();
        if (count($result) > 1) {
            throw new Exception(xarML('Found a non-unique category name'));
        }
    ...
    {
        if (empty($id)) {
            throw new Exception(xarML('No id passed to getInfo'));
        }
    ...
    public function delete($id = 0)
    {
        ...
        if (empty($this_category[$this->parent]) || ($this_category['id'] == 1) || ($this_category[$this->left] == 1)) {
            return false;
        }
    ...
    public function appendTree($itemid, $args = [])
    {
        ...
        foreach ($descendents as $key => $child) {
            ...
            if ($child[$this->parent] != (int) $root['id']) {
                $child[$this->parent] = $oldnewids[$child[$this->parent]];
            }
