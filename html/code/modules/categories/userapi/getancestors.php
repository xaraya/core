...
/**
 * categories userapi getancestors function
...
class GetancestorsMethod extends MethodClass
{
     ...
     * @param mixed $args ['self'] =Boolean= return the cid itself (default true)
     * @param mixed $args ['return_itself'] alias of 'self'
     * @param mixed $args ['order'] 'root' or 'self' first; default 'root' (i.e. oldest ancestor first)
     * @param mixed $args ['descendants'] array to determine how descendants will be returned: 'tree', 'lists', 'list', ['none']
     * @staticvar array $cached
     * @return bool|array|void Returns array of category info or false on failure
    ...
    public function __invoke(array $args = [])
    {
        ...
        foreach ($cids as $loopcid) {
            if (!isset($cached[$loopcid])) {
                array_push($dbcids, $loopcid);
            }
        }
