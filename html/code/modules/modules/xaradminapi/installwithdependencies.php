    ...
    switch ($args['phase']) {
        ...
        case 0:
            ...
            if (!$initialised && file_exists(sys::code() . 'modules/' . $modInfo['osdirectory'] . '/xartemplates/includes/installoptions.xt')) {
                xarController::$response->redirect(xarModURL('modules','admin','modifyinstalloptions',array('regid' => $mainId)));
                return true;
            } else {
        ...
        case 1:
            ...
            if (empty($modstack)) {
                ...
                xarController::$response->redirect(xarModURL('modules', 'admin', 'list', array('state' => 0), NULL, $target));
            } else {
                // Do the next module
