 ...
 */
class ShowprivilegesMethod extends MethodClass
{
    ...
    public function __invoke(array $args = [])
    {
        ...
        foreach ($ancestors as $ancestor) {
            if ($ancestor->getLevel() > $maxlevel) {
                $maxlevel = $ancestor->getLevel();
            }
        ...
        foreach ($curprivs as $priv) {
            ...
            $curprivileges = array_merge([$priv], $curprivileges);
        ...
        for ($i = 1;$i < $maxlevel + 1;$i++) {
            ...
            foreach ($privilegestodo as $todo) {
                ...
                foreach ($privilegesdone as $done) {
                    if (!($done['relation'] < $todo['relation'])) {
                        continue;
                    }
