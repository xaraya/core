 ...
 */
class SenduseremailMethod extends MethodClass
{
    ...
    public function __invoke(array $args = [])
    {
        ...
        if (!isset($mailtype)) {
            throw new EmptyParameterException('mailtype');
        }
        ...
        foreach ($id as $userid => $val) {
            ///get the user info
            $user = $userapi->get(['id' => $userid, 'itemtype' => xarRoles::ROLES_USERTYPE]);
            if (!isset($pass)) {
                $pass = '';
            }
            if (!isset($ip)) {
