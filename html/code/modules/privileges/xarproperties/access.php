...
class AccessProperty extends DataProperty
{
    ...
    public $failure     = 0;
    public $myself      = -6;
    public $allallowed  = ['Administrators'];
    ...
    public function checkInput($name = '', $value = null)
    {
        ...
        $dropdown = $this->prop()->getProperty(['name' => 'dropdown']);
        $value = [];
        ...
        if ($this->initialization_group_multiselect) {
            ...
            if (!$multiselect->checkInput($name . '_group')) {
                return false;
            }
            ...
            if (!$dropdown->checkInput($name . '_group')) {
                return false;
            }
        ...
        if (!$dropdown->checkInput($name . '_level')) {
            return false;
        }
        ...
        if (!$dropdown->checkInput($name . '_failure')) {
            return false;
        }
    ...
    public function showInput(array $data = [])
    {
        ...
        if (!isset($value['group'])) {
            $value['group'] = $data['group_multiselect'] ? [0] : 0;
        }

        if (!isset($data['groupoptions'])) {
            $data['groupoptions'] = $this->getgroupoptions();
        }
        if (!isset($data['leveloptions'])) {
    ...
    public function showOutput(array $data = [])
    {
        ...
        $value = $this->getValue();
        if (!isset($data['level'])) {
            $data['level'] = (isset($value['level'])) ? $value['level'] : 800;
        }
        if (!isset($data['group'])) {
            $data['group'] = (isset($value['group'])) ? $value['group'] : [];
        }
        if (!isset($data['failure'])) {
            $data['failure'] = (isset($value['failure'])) ? $value['failure'] : 1;
        }
     ...
     */
    public function getfailureoptions()
    {
        $options = [
            ['id' => 0, 'name' => $this->ml('Fail silently')],
            ['id' => 1, 'name' => $this->ml('Throw exception')],
        ];
        return $options;
    }
