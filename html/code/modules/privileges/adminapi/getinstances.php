 ...
 */
class GetinstancesMethod extends MethodClass
{
    ...
    public function __invoke(array $args = [])
    {
        ...
        $bindvars = [$modid,$component];

        $instances = [];
        $stmt = $dbconn->prepareStatement($query);
        $result = $stmt->executeQuery($bindvars);
        while ($result->next()) {
            [$header, $selection, $ddlimit] = $result->fields;
            ...
            if (preg_match('/^xarMod::apiFunc/i', $selection)) {
                ...
                while ($result1->next()) {
                    [$dropdownline] = $result1->fields;
                    if (($dropdownline != 'All') && ($dropdownline != 'None')) {
                        $dropdown[] = ['id' => $dropdownline, 'name' => $dropdownline];
                    }
                }
            ...
            $instances[] = ['header' => $header,'dropdown' => $dropdown, 'type' => $type];
        }
