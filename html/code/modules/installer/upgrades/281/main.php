...
function main_upgrade_281()
{
    $data['upgrade']['message'] = xarML('The upgrade to version 2.8.1 was successfully completed');
    $data['upgrade']['tasks'] = [];

    $upgrades = [
        'sql_281_01', // Upgrading the core module version numbers
    ];
    foreach ($upgrades as $upgrade) {
        if (!xarUpgrader::loadFile('upgrades/281/database/' . $upgrade . '.php')) {
            $data['upgrade']['tasks'][] = [
                'reply' => xarML('Failed!'),
