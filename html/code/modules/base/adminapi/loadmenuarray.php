 ...
 */
class LoadmenuarrayMethod extends MethodClass
{
     ...
     * @var string $args [modname] optional name of module to get links for (default current request module)<br/>
     * @var string $args [modtype] optional type of links to return [admin|user] (default current request type)<br/>
     * @var string $args [layout] return links for menu or links for tabs with menu title info (default links)<br/>
     * @var bool $args [noxml] optionally force looking for links only from a getmenulinks function<br/>
     * @var bool $args [nolinks] optionally force looking for links only from xml files
     * @return array menulinks for the module
     * @see AdminApi::loadmenuarray()
     */
    public function __invoke(array $args = [])
    {
        if (!isset($args['modname']) || !isset($args['modtype']) || !isset($args['funcname'])) {
            ...
            if (empty($args['modtype'])) {
                $args['modtype'] = $urlinfo[1];
            }
            // handle modules using util as an admin type
            if ($args['modtype'] == 'util') {
                $args['modtype'] = 'admin';
            }
            if (empty($args['funcname'])) {
                $args['funcname'] = $urlinfo[2];
            }
            if (empty($args['urlargs'])) {
                $args['urlargs'] = [];
            }
        }
