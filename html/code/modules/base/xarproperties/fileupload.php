...
class FileUploadProperty extends DataProperty
{
    ...
    public function __construct(ObjectDescriptor $descriptor)
    {
        ...
        if ($this->mem()->get('Hooks.uploads', 'ishooked')) {
            ...
            if ($this->mod()->isHooked('uploads', $this->req()->getModule())) {
                $this->UploadsModule_isHooked = true;
            }
            /*
            $list = $this->hooked()->getSubjectObservers($this->req()->getModule(), 'ItemTransform');
        ...
        /*
                if($this->req()->getServerVar('PATH_TRANSLATED')) {
                    $basepath = dirname(realpath($this->req()->getServerVar('PATH_TRANSLATED')));
                } elseif($this->req()->getServerVar('SCRIPT_FILENAME')) {
                    $basepath = dirname(realpath($this->req()->getServerVar('SCRIPT_FILENAME')));
                } else {
                    $basepath = './';
                }
        ...
        */
        ...
        $this->initialization_basedirectory = preg_replace('/\{user_theme\}/', "themes/" . $this->mod('themes')->getVar('default_theme'), $this->initialization_basedirectory);
        $this->initialization_basedirectory = preg_replace('/\{admin_theme\}/', "themes/" . $this->mod('themes')->getVar('admin_theme'), $this->initialization_basedirectory);
        $this->initialization_basedirectory = preg_replace('/\{theme\}/', $this->tpl()->getThemeDir(), $this->initialization_basedirectory);
        ...
        if ($this->UploadsModule_isHooked == true) {
            ...
            if (!empty($this->initialization_basedirectory) || !empty($this->initialization_importdirectory)) {
                ...
                if (!empty($this->initialization_basedirectory)) {
                    $override['upload'] = ['path' => $this->initialization_basedirectory];
                }
                if (!empty($this->initialization_importdirectory)) {
                    $override['import'] = ['path' => $this->initialization_importdirectory];
                }
            } else {
                $override = null;
            }

            $return = $this->mod()->apiFunc(
                'uploads',
                'admin',
                'validatevalue',
                ['id' => $name, // not $this->id
        ...
        if (isset($file['tmp_name']) && is_uploaded_file($file['tmp_name']) && $file['size'] > 0 && $file['size'] < $this->validation_max_file_size) {
            if (!empty($file['name'])) {
                ...
                if ($this->validation_sanitize_filename) {
                    $file['name'] = $this->mod()->apiFunc('base', 'admin', 'sanitize_filename', ['filename' => $file['name']]);
                }
    ...
    public function showInput(array $data = [])
    {
        ...
        if (isset($data['maxsize'])) {
            $this->validation_max_file_size = $data['maxsize'];
        }
        ...
        if ($this->remove_leftover_values && !empty($data['value']) && (is_numeric($data['value']) || stristr($data['value'], ';'))) {
            $data['value'] = '';
        }
    ...
    public function showOutput(array $data = [])
    {
        ...
        if (!isset($data['basedirectory'])) {
            $data['basedirectory'] = $this->initialization_basedirectory;
        }
