...
class SelectProperty extends DataProperty
{
    ...
    {
        if (isset($data['option_link'])) {
            $this->display_option_link = $data['option_link'];
        }
        if (isset($data['value'])) {
        ...
        if (!empty($result) && is_string($result)) {
            $result = $this->var()->prep($result);
        }
        if (!empty($data['link'])) {
            $data['option'] = ['id' => $this->value, 'name' => $result, 'link' => $data['link']];
        } else {
            $data['option'] = ['id' => $this->value, 'name' => $result];
        }
        ...
        if (!empty($this->initialization_function)) {
            ...
            if (is_array(reset($items))) {
                foreach ($items as $id => $name) {
                    $options[] = ['id' => $name['id'], 'name' => $name['name']];
                }
            } else {
                foreach ($items as $id => $name) {
    ...
    private function getTemplateData(array $data = [])
    {
        ...
        if (!is_array(current($data['options']))) {
            ...
            foreach ($data['options'] as $key => $value) {
                $normalizedoptions[] = ['id' => $key, 'name' => $value];
            }
        ...
        if (!empty($data['value']) && $this->validation_override) {
            ...
            if (!$found) {
                $data['options'][] = ['id' => $data['value'], 'name' => $data['value']];
            }
        }
        // optionally add hidden previous_value field
        if (!isset($data['previousvalue'])) {
            $data['previousvalue'] = false;
        }
