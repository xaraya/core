...
class xarTplPager extends xarObject
{
    ...
    public static function getInfo($currentItem, $total, $itemsPerPage = 10, $blockOptions = 10)
    {
        ...
        if (is_array($blockOptions)) {
            if (!empty($blockOptions['blocksize'])) {
                $blockSize = $blockOptions['blocksize'];
            }
            if (!empty($blockOptions['firstitem'])) {
                $firstItem = $blockOptions['firstitem'];
            }
            if (!empty($blockOptions['firstpage'])) {
                $firstPage = $blockOptions['firstpage'];
            }
            if (!empty($blockOptions['urltemplate'])) {
                $urltemplate = $blockOptions['urltemplate'];
            }
        ...
        // Default values.
        if (empty($blockSize) || $blockSize < 1) {
            $blockSize = 10;
        }
        if (empty($firstItem)) {
            $firstItem = 1;
        ...
        if ($xar->mem()->get('Pager.core', 'request') == $request) {
            return $xar->mem()->get('Pager.core', 'details');
        }

        // Record the values in this request.
        $xar->mem()->set('Pager.core', 'request', $request);
        ...
        if ($blockLastItem > $lastItem) {
            $blockLastItem = $lastItem;
        }
        ...
        for ($i = $blockFirstItem; $i <= $blockLastItem; $i += $itemsPerPage) {
            if (!empty($urltemplate)) {
                $data['middleurls'][$pageNum] = str_replace([$urlItemMatch,$urlItemMatchEnc], $i, $urltemplate);
            }
            $data['middleitems'][$pageNum] = $i;
            $data['middleitemsfrom'][$pageNum] = $i;
            $data['middleitemsto'][$pageNum] = $i + $itemsPerPage - 1;
            if ($data['middleitemsto'][$pageNum] > $total) {
                $data['middleitemsto'][$pageNum] = $total;
            }
            $pageNum += 1;
        }
        ...
        if (!empty($urltemplate)) {
            ...
            $data['firsturl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['firstpage'], $urltemplate);
            $data['lasturl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['lastpage'], $urltemplate);
        }
        ...
        if ($currentPage > $firstPage) {
            ...
            if (!empty($urltemplate)) {
                $data['prevpageurl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['prevpage'], $urltemplate);
            }
        } else {
        ...
        if ($pageLastItem < $lastItem) {
            ...
            if ($nextPageLastItem > $lastItem) {
                $nextPageLastItem = $lastItem;
            }
            $data['nextpageitems'] = ($nextPageLastItem - $pageLastItem);
            $data['nextpage'] = ($pageLastItem + 1);
            if (!empty($urltemplate)) {
                $data['nextpageurl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['nextpage'], $urltemplate);
            }
        } else {
        ...
        if ($currentBlock > $firstBlock) {
            ...
            if (!empty($urltemplate)) {
                $data['prevblockurl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['prevblock'], $urltemplate);
            }
        } else {
        ...
        if ($currentBlock < $totalBlocks) {
            ...
            if ($nextBlockLastItem > $lastItem) {
                $nextBlockLastItem = $lastItem;
            }
            $data['nextblockpages'] = ceil(($nextBlockLastItem - $blockLastItem) / $itemsPerPage);
            $data['nextblock'] = ($blockLastItem + 1);
            if (!empty($urltemplate)) {
                $data['nextblockurl'] = str_replace([$urlItemMatch,$urlItemMatchEnc], $data['nextblock'], $urltemplate);
            }
        } else {
        ...
        $xar->mem()->set('Pager.core', 'details', $data);
     ...
     *
     */
    public static function getPager($startNum, $total, $urltemplate, $itemsPerPage = 10, $blockOptions = [], $template = 'default', $tplmodule = 'base')
    {
        ...
        if (is_numeric($blockOptions)) {
            $blockOptions = ['blocksize' => $blockOptions];
        }
        ...
        if ($data['prevblockpages'] > 0) {
            $xar->mem()->set('Pager.first', 'leftarrow', $data['firsturl']);
        }

        // Links for next block of pages.
        if ($data['nextblockpages'] > 0) {
            $xar->mem()->set('Pager.last', 'rightarrow', $data['lasturl']);
        }
