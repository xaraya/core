...
class Themes_MetaBlock extends BasicBlock
{
        ...
        switch ($oldversion) {
            case '0.0.0':
                ...
                if (isset($data['metakeywords'])) {
                    unset($data['metakeywords']);
                }
                if (isset($data['usegeo'])) {
                    unset($data['usegeo']);
                }
            ...
            case '2.2.0':
                ...
                $linktags = [
                    ['rel' => 'author', 'href' => !empty($data['authorpage']) ? $data['authorpage'] : '[baseurl]', 'title' => $this->prep()->text($author), 'type' => 'text/html'],
                    ...
                    ['rel' => 'Top', 'href' => '[baseurl]', 'title' => '', 'type' => 'text/html'],
                    ['rel' => 'parent', 'href' => '[baseurl]', 'title' => '', 'type' => 'text/html'],
                    ...
                    ['rel' => 'alternate', 'href' => '[currenturl]theme=rss', 'title' => 'RSS-feed', 'type' => 'application/rss+xml'],
                    //array('rel' => 'service.feed', 'href' => '[currenturl]theme=atom', 'title' => 'Atom-feed', 'type' => 'application/atom+xml'),
                    ['rel' => 'alternate', 'href' => '[currenturl]theme=print', 'title' => 'Print', 'type' => 'text/html'],
                ];
                // remove deprecated property values
                if (isset($data['authorpage'])) {
                    unset($data['authorpage']);
                }
                if (isset($data['copyrightpage'])) {
                    unset($data['copyrightpage']);
                }
    ...
    public function parseLinkTags()
    {
        ...
        if (!empty($this->linktags)) {
            ...
            foreach ($this->linktags as $tag) {
                ...
                if (empty($tag['rel']) || empty($tag['href'])) {
                    continue;
                }
                if (in_array($tag['href'], $deprecated)) {
                    continue;
                }
                if (!$tag['url'] = $this->_decodeURL($tag['href'])) {
                    continue;
                }
                $linktags[] = $tag;
            }
    ...
    public function _decodeURL($url)
    {
        ...
        if (strpos($url, '[') === 0) {
            ...
            $decoded_url = $this->ctl()->withXMLURLs() ? $this->prep()->text($url) : $url;
        }
    ...
    public function default_linktags()
    {
        $author = $this->mod('themes')->getVar('SiteName');
        $linktags = [
            ['rel' => 'author', 'href' => '[baseurl]', 'title' => $this->prep()->text($author), 'type' => 'text/html'],
