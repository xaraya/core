...
class xarJS extends xarObject
{
        ...
        foreach ($paths as $path) {
            ...
            foreach (array_keys($folders) as $lib) {
                ...
                if (!isset($this->local_libs[$lib])) {
                    $this->local_libs[$lib] = new xarJSLib($lib, $xar);
                }
                ...
                if ($info['origin'] == 'local') {
                    ...
                    if ($hasplugins) {
                        ...
                        foreach ($this->local_libs[$info['lib']]->plugins as $name => $plugin) {
                            ...
                            if (!empty($version)) {
                                $registerargs['version'] = $version;
                            }
                            if (!empty($file)) {
                                $registerargs['file'] = $file;
                            }
                            if (!empty($style)) {
                                $registerargs['style'] = $style;
                            }
    ...
    **/
    private function getLibInfo($lib, $vers = '', $src = '')
    {
                    ...
                    foreach ($scopearray as $package => $packagearray) {
                        foreach ($packagearray as $dir => $dirarray) {
                            foreach ($dirarray as $file => $filearray) {
                                if (!empty($src) && ($filearray['src'] != $src)) {
                                    continue;
                                }
                                if (!empty($vers) && ($filearray['version'] != $vers)) {
                                    continue;
                                }
        ...
        foreach ($this->remote_libs as $thislib) {
            ...
            if (!empty($lib) && ($thislib['lib'] != $lib)) {
                continue;
            }
            if (!empty($src) && ($thislib['src'] != $src)) {
                continue;
            }
            if (!empty($vers) && ($thislib['version'] != $vers)) {
                continue;
            }
            $candidates[] = $thislib;
        }
    ...
    **/
    private function getPluginInfo($lib, $plug, $vers = '', $file = '', $style = '')
    {
        ...
        foreach ($this->remote_libs as $thislib) {
            if ($thislib['type'] != 'plugin') {
                continue;
            }
            if (!empty($lib) && ($thislib['parent'] != $lib)) {
                continue;
            }
            if (!empty($vers) && ($thislib['version'] != $vers)) {
                continue;
...
class xarJSLib extends xarObject
{
                    ...
                    foreach (array_keys($versions) as $version) {
                        ...
                        foreach ($files as $folder => $items) {
                            foreach ($items as $file => $filepath) {
                                ...
                                switch ($ext) {
                                    case 'js':
                                        $this->plugins[$plugin][$version][$file] = [
                                            'src' => $filepath,
                                            'version' => $version,
                                            'origin' => 'local',
                                        ];
    ...
    **/
    private function loadXml($xmlPath)
    {
        ...
        if (!$string || !function_exists('simplexml_load_string')) {
            return;
        }
