...
class RelationalDataStore extends SQLDataStore
{
    ...
    public function itemExists(array $args = [])
    {
        ...
        if (empty($this->object->primary)) {
            throw new Exception($this->ml('The object #(1) has no primary key', $this->object->name));
        }
        ...
        // Run it
        if (!$q->run()) {
            throw new Exception($this->ml('Query failed'));
        }
        $result = $q->output();
        return !empty($result);
    ...
    public function createItem(array $args = [])
    {
        ...
        try {
            ...
            $message = $this->ml('The following notional query failed:<br/>');
            $message .= $q->tostring();
            $message .= $this->ml('<br/>The specific message was:<br/>');
    ...
    public function getItems(array $args = [])
    {
        ...
        // Make sure we have a primary field
        //        if (empty($this->object->primary)) throw new Exception($this->ml('The object #(1) has no primary key', $this->object->name));
    ...
    private function setItemValue($itemid, $row, $field, $object, $fordisplay = 0, $row_output = 'associative')
    {
        ...
        if (in_array($object->properties[$field]->type, self::$_subitems_types)) {
            ...
            if (!is_object($subitemsobject)) {
                throw new Exception($this->ml('The property #(1) has no valid subitems object. Value is: #(2)', $field, $subitemsobject));
            }
    ...
    private function addqueryfields(Query $query, $objectname)
    {
        ...
        $object = $xar->data()->getObject(['name' => $objectname]);
