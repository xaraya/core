...
class xarLog extends xarObject
{
    ...
    public const LEVEL_ERROR      = 8;
    public const LEVEL_WARNING    = 16;
    public const LEVEL_NOTICE     = 32;
    public const LEVEL_INFO       = 64;
    public const LEVEL_DEBUG      = 128;
    // This is a special define that includes all the levels defined above
    public const LEVEL_ALL        = 255;
    ...
    public static function init(array $args = [])
    {
        ...
        try {
            ...
            if (!$sysConfig->getVar('Log.Enabled')) {
                return true;
            }
        } catch (Exception $e) {
            return true;
        ...
        if (self::configReadable()) {
            ...
            foreach ($availables as $available) {
                ...
                $config = ['type' => $available, 'fallback' => false];
        ...
        if ((int) $sysConfig->getVar('Log.Enabled') && empty($availables) && self::fallbackPossible()) {
            ...
            if ($logFile) {
                $levels = @unserialize((string) $sysConfig->getVar('Log.Level'));
     ...
     */
    public static function configReadable()
    {
     ...
     */
    public static function fallbackFile()
    {
        ...
        $logFile = sys::varpath() . '/logs/' . $sysConfig->getVar('Log.Filename');
    ...
    public static function availables()
    {
        $sysConfig = self::sysConfig();
        // Get the available loggers as an array
        $availables = $sysConfig->getVar('Log.Available');
        if (!empty($availables)) {
     ...
     *
     */
    public static function message($message, $level = self::LEVEL_DEBUG)
    {
        if (empty($level)) {
            $level = xarLog::LEVEL_DEBUG;
        }
        if (($level == self::LEVEL_DEBUG) && !xarCore::isDebuggerActive()) {
            return;
        }
    ...
    public static function variable($name, $var, $level = '')
    {
        if (empty($level)) {
