...
class xarRequest extends xarObject
{
    ...
    public function __construct($url = null, $xar = null)
    {
        ...
        // xar::req()->getRequest() comes after xar::load()
     ...
     *
     * index.php/<module>/<something mapped to UserGui method of that module>, or
     * index.php/<module>/admin/<something mapped to AdminGui method>
     *
     * We rely on method iController::decode() to translate PATH_INFO
     * into something the module can work with for the input variables.
     * On output, the short URLs are generated by iController::encode(),
     * that is called automatically by xar::ctl()->getModuleURL().
     *
    ...
    public function getInfo($url = '')
    {
        ...
        if (is_array($this->currentRequestInfo) && empty($url)) {
            ...
            $xar->log()->info('Avoiding loop in xar::req()->getRequest()->getInfo()');
            return $loopHole;
        }
        // Get variables
        if (empty($url)) {
