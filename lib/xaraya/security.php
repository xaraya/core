 ...
 * @return  bool
 */
function xarMakeGroup($name,$uname='') { return Roles_Master::makeGroup($name,$uname); }
...
{
    return Roles_Master::makeUser($name,$uname,$email,$pass,$dateReg,$valCode,$state,$authModule);
}
...
{
    return Roles_Master::makeMemberByName($childName, $parentName);
}
...
{
    $parent = Roles_Master::ufindRole($parentName);
    $child = Roles_Master::ufindRole($childName);
...
{
    $parent = Roles_Master::getRole($parentId);
    $child = Roles_Master::getRole($childId);
 ...
 */
function xarGetGroups() { return Roles_Master::getgroups(); }
 ...
 * @return  object role
 */
function xarFindRole($name) { return Roles_Master::findRole($name);  }
function xarUFindRole($name){ return Roles_Master::ufindRole($name); }

function xarCurrentRole()
{
    return Roles_Master::getRole(xarSessionGetVar('role_id'));
}

function xarIsParent($name1, $name2)
{
    $role1 = Roles_Master::findRole($name1);
    $role2 = Roles_Master::ufindRole($name2);
...
{
    return Privileges_Master::xarSecLevel($levelname);
}
...
{
    if ($mask == "All") $mask = 0;
    $mask = Privileges_Master::getMask($name,$module,$component,true);
    if ($mask) return true;
    else return false;
}
...
{
    if ($mask == "All") $mask = 0;
    return Privileges_Master::querymask($mask, $component, $instance, $module, $role,$pnrealm,$pnlevel);
}
...
function xarSecurityCheck($mask, $showException=1, $component='', $instance='', $module='', $role='',$pnrealm=0,$pnlevel=0)
{
    ...
    if(isset($installing) && ($installing == true)) {
       ...
       return Privileges_Master::xarSecurityCheck($mask, $showException, $component, $instance, $module, $role,$pnrealm,$pnlevel);
    }
}
...
{
    return Privileges_Master::register($name,$realm,$module,$component,$instance,xarSecurityLevel($level),$description);
}
...
{
    return Privileges_Master::unregister($name);
}
...
{
    if ($module == "All") {
        $modid = Privileges_Master::PRIVILEGES_ALL;
    } elseif ($module == null) {
        $modid = null;
    } else {
        $modid = xarMod::getID($module);
    }
    return Privileges_Master::removeMasks($modid);
}
